...
public class AtaCommand implements CommandFactory {
    ...
    public Optional<TargetCommand> makeCommand(RequestContext ctx) {
        try {
            ...
            AtaPayload query = AtaPayload.newPacket(ctx.getAoeFrame().getPayload());
    ...
    public TargetCommand write(RequestContext ctx, AtaPayload query) {
        return target -> {
            LOG.debug(
                    "ATA write{} requested for device {}",
                    query.getHeader().isAsync() ? "[async]" : "",
                    target);
            AtaResponse response = ataResponseFactory.apply(ctx, target);
    ...
    public TargetCommand identify(RequestContext ctx, AtaPayload query) {
        return target -> {
            ...
            HdDriveId driveIdentity =
                    new HdDriveId(deviceConfig.getTarget(), deviceConfig.getSizeInBytes());
            AtaResponse response =
                    ataResponseFactory.apply(ctx, target).setPayload(driveIdentity.toByteArray());
